...
| `data_prep.py` | the corpus for the training pie: a mixed pretraining text with `sources: {name: tokens}` in RESULT (the Lab's pie), a reasoning-only text for mid-training, SFT and DPO rows from your Traces plus a synthetic reasoning set (two-step arithmetic, transitive comparisons, modus ponens/tollens) with verifiable answers, and a held-out eval set; one line format `q: ...\na: <chain> answer: <gold>` shared by every later stage | 21 | `python lab/recipes/data_prep.py --smoke` | `python lab/recipes/data_prep.py --traces-jsonl out/traces_all.jsonl --n-reason 20000 --tokenizer gpt2 --hf-dataset roneneldan/TinyStories --max-samples 20000` | `modal run lab/recipes/modal_app.py --recipe data_prep --args "--n-reason 20000"` |
| `grpo_reason.py` | GRPO with a verifiable-answer reward (`0.2 * format + 0.8 * correct`, scored on the span after `answer:`) on data_prep's reasoning set; the same loop as grpo_tool.py with a different `score()`; `--ckpt` from sft_lora.py selects the nano path; TRL GRPOTrainer in real mode | 21 | `python lab/recipes/grpo_reason.py --smoke --steps 30` | `python lab/recipes/grpo_reason.py --model Qwen/Qwen2.5-0.5B-Instruct --tasks-jsonl out/data_prep/reason_train.jsonl --steps 200 --group 8` | `modal run lab/recipes/modal_app.py --recipe grpo_reason --args "--steps 200 --group 8"` |
...
The training pie (Lab 21, `server/pipeline.py`, the Lab's Pipeline tab) chains these
as one DAG of runs: `data_prep` -> `pretrain_nano --corpus` -> `midtrain --ckpt --text-a --text-b` ->
`sft_lora --ckpt --pairs-jsonl` -> `grpo_reason --ckpt --tasks-jsonl` -> `eval_suite --ckpt --baseline-ckpt --custom-jsonl`.
A lab checkpoint (`--ckpt`) selects the nano-GPT path of `sft_lora`, `grpo_reason` and `eval_suite` even without
`--smoke`; `pretrain_nano --init` continues a checkpoint. Paths flow forward from each stage's RESULT line.
